...
Claude-Code-specific behavior on top of the shared AGENTS.md policy.

### Task tracking

- Use the native task tools (`TaskCreate`, `TaskUpdate`, `TaskList`, `TaskGet`) to break multi-step work into trackable items. Mark each task `in_progress` BEFORE starting and `completed` immediately after finishing.
- Match task scope to a single shippable unit — not too coarse, not too fine.

### Plan mode

- Use `EnterPlanMode` for non-trivial implementation tasks. Read-only exploration inside plan mode; write the plan to the plan file; call `ExitPlanMode` to request approval.
- Don't ask for plan approval via prose or `AskUserQuestion` — `ExitPlanMode` is the approval channel.

### Skills and slash commands

- `/loop`, `/ultrareview`, and other slash commands are user-triggered; do not auto-invoke them.
- The `Skill` tool invokes available skills from the conversation's skills list — only invoke skills present in that list. Don't fabricate skill names.

### MCP / external tools

- MCP tools surface as `mcp__<server>__<tool>` — load schema via `ToolSearch` before calling if not pre-listed.
...
- Persistent memory in `~/.claude/projects/-Users-vero-workspace-beegy/memory/`. Save corrections AND confirmed approaches (not just corrections). Index entries in `MEMORY.md` (≤150 chars per line).

### Plan file location

- Plan mode writes to the plan file referenced in the plan-mode system message. Only that file is writable in plan mode.
