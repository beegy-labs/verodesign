...
export async function walk(dir: string, exts?: string[], out: string[] = []): Promise<string[]> {
  for (const name of await readdir(dir)) {
    ...
    if (!exts || exts.includes(extname(name))) out.push(full);
  }
  return out;
}
...
export async function rewriteFile(
  file: string,
  mutate: (input: string) => string | Promise<string>,
  dry = false
): Promise<boolean> {
