...
Gemini-CLI-specific behavior placeholder. Populate when Gemini CLI is brought online.

### Response style (reinforces AGENTS body)

- BLUF + 3-section briefing (`목적 / 변경범위 / 이펙트`) for non-code answers. Per [response-style-policy.md](docs/llm/policies/response-style-policy.md).

### Tool conventions

- Gemini CLI uses its own context-file conventions (e.g. `GEMINI.md` at repo root, `.gemini/` subdirectory if introduced).
- Memory / session state — TBD per Gemini CLI version. Persist durable lessons as ADRs in `docs/llm/decisions/`, not in tool-local memory.
