...
export const TOKEN_REF_RENAMES = [
...
const STATUS_KEYS = ['success', 'warning', 'error', 'info', 'neutral'];
const WRAPPED_KEYS = ['primary', 'accent', 'accent-2', 'accent-3', 'destructive', 'cancelled'];
...
function moveWrappedForegrounds(theme) {
...
export function migrateTokenJson(source) {
...
export function migrateText(source) {
    let out = source;
    for (const [from, to] of CSS_VAR_RENAMES)
